...
maps. This module deliberately stays close to the package format so downstream
extractors can decide which semantics they need.
"""
...
def _parse_xml(data: bytes, part_name: str) -> etree._Element | None:
...
def _parse_relationships(root: etree._Element) -> dict[str, DocxRelationship]:
    rels: dict[str, DocxRelationship] = {}
    for rel in root.findall(f"{{{REL_NS}}}Relationship"):
        rel_id = rel.get("Id")
        if not rel_id:
            continue
        rels[rel_id] = DocxRelationship(
            rel_id=rel_id,
...
def read_docx_package(source: str | Path | bytes | BinaryIO) -> DocxPackage:
    """
    Read a .docx file into parsed XML parts, binary parts, and relationships.

    Args:
        source: File path, raw docx bytes, or a binary file-like object.

    Returns:
        DocxPackage with all parseable XML parts and package relationships.
    """
    ...
    relationships: dict[str, dict[str, DocxRelationship]] = {}
    ...
    with ZipFile(zip_source) as zf:
        ...
        for name in part_names:
            ...
            if norm_name.endswith(".xml") or norm_name.endswith(".rels"):
                root = _parse_xml(data, norm_name)
                if root is None:
