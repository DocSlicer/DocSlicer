...
Production rebuild of the gutter detector. Pipeline:
    ...
    8. merge_gutters_onto_words  annotate each word with the kept gutters
                             bordering it (gutter_id_left/right,
                             reading_column)
...
Public API:
    df_words, df_rects = detect_gutters(df_words, df_shapes, df_images,
    ...
    df_words     = merge_gutters_onto_words(df_words, df_rects, config)
...
class GutterConfig:
    ...
    enumeration_workers: int = 0        # process pool size for the per-page search;
                                        # 0 = auto (min(8, cpu_count)), 1 = always sequential
    enumeration_min_obstacles: int = 20_000  # pool only pays off past this many in-bound obstacles
                                             # document-wide (search cost tracks obstacles, not pages;
                                             # below it, pool spawn costs more than it saves)
    # ---- step 4: hard gates ----
    ...
    # ---- step 8: merge onto words ----
    eps: float = 1e-6                       # strict-overlap epsilon: touching edges do not count as overlap
...
def _enumerate_pages_chunk(
    ...
    """Pool worker: run the branch-and-bound for a chunk of (page, bound, obs) jobs."""
    return [
        (page, _enumerate_max_rects(bound, obs, min_w, min_h, max_rects, max_expansions))
        for page, bound, obs in jobs
    ]
...
def enumerate_rects(
    ...
    Pages are independent, so on documents carrying at least
    enumeration_min_obstacles in-bound obstacles the per-page searches run
    on a process pool (enumeration_workers; page order interleaved across
    chunks so dense pages spread out).  Inputs
    and outputs per page are tiny (an obstacle array in, at most
    max_rects_per_page rects out), and results are reassembled in page order
    — output is identical to the sequential path.  Any pool failure (nested
    daemonic process, spawn refusal, ...) falls back to sequential.
    ...
    for row in df_bounds.itertuples(index=False):
        ...
        jobs.append((row.page_number, bound, obs))

    args = (
        config.min_rect_width, config.min_rect_height,
        config.max_rects_per_page, config.max_node_expansions,
    )
    n_workers = config.enumeration_workers or min(8, os.cpu_count() or 1)
    n_obstacles = sum(obs.shape[0] for _, _, obs in jobs)
    page_rects: list | None = None
    if n_workers > 1 and n_obstacles >= config.enumeration_min_obstacles:
        try:
            chunks = [jobs[i::n_workers * 4] for i in range(n_workers * 4)]
            with ProcessPoolExecutor(max_workers=n_workers) as ex:
                futures = [ex.submit(_enumerate_pages_chunk, c, *args) for c in chunks if c]
                page_rects = [pr for f in futures for pr in f.result()]
            page_rects.sort(key=lambda pr: pr[0])  # chunk interleave -> back to page order
        except Exception:
            page_rects = None  # e.g. daemonic child, spawn refusal: fall back below

    if page_rects is None:
        page_rects = _enumerate_pages_chunk(jobs, *args)
...
# =======================================================================================================================
# Step 8: merge gutters onto words
# =======================================================================================================================

def merge_gutters_onto_words(
    df_words: pd.DataFrame,
    df_gutters: pd.DataFrame,
    config: GutterConfig = GutterConfig(),
) -> pd.DataFrame:
    """
    Step 8: annotate each word with the kept gutters that border it.
    ...
    within config.side_touch_tol of a word edge still counts as beside it
    (guards float jitter at touching edges).  Y-overlap, conversely, is tested
    against each gutter shrunk inward by config.flank_y_pad on its top and
    bottom: a text line that only grazes the gutter's top/bottom edge (bbox
    slop, not an actual column of text beside it) must not register as
    flanking.

    Same per-page numpy broadcasting as steps 4-6 (words x gutters); no
    per-word loops.
    """
    ...
    if have_gutters:
        ...
        for page_number, widx in out.groupby("page_number", sort=False).indices.items():
            ...
            # Shrink each gutter's y-range inward by config.flank_y_pad before
            # testing overlap, so a word merely grazing the gutter's top/bottom
            # edge does not count as flanking it (clamped to the gutter's
            # midline so this never inverts a gutter shorter than
            # 2 * config.flank_y_pad).
            g_ymid = (gy0 + gy1) * 0.5
            gy0_pad = np.minimum(gy0 + config.flank_y_pad, g_ymid)
            gy1_pad = np.maximum(gy1 - config.flank_y_pad, g_ymid)

            # words x gutters: y-overlap, then which side each gutter is on
            yov = (gy0_pad[None, :] < wy1[:, None] - config.eps) & (gy1_pad[None, :] > wy0[:, None] + config.eps)
            left_of = yov & (gx1[None, :] <= wx0[:, None] + config.side_touch_tol)
            right_of = yov & (gx0[None, :] >= wx1[:, None] - config.side_touch_tol)
...
def detect_gutters(
    ...
    """
    Run the full pipeline (steps 1-8) and return (df_words, df_gutters):
    words annotated with the gutters bordering them, and the scored
    whitespace rectangles themselves.
    ...
    viewer (gutter_keep marks the final survivors) — merge_gutters_onto_words
    still only honors gutter_keep == True rects either way.
    """
    if df_words is None or df_words.empty:
        empty_gutters = pd.DataFrame(columns=[*_RECT_COLS, *_GATE_COLS,
                                              "flank_sources_left", "flank_sources_right",
                                              "gutter_score", "gutter_keep"])
        return df_words, empty_gutters
    ...
    df_words_out = merge_gutters_onto_words(df_words, df_rects, config)
    return df_words_out, df_rects
