...
Public API:
    df_words, df_gutters = detect_gutters(df_words, df_shapes)      # stage 1 + 2 + 3
...
def score_gutters(
    ...
    for page_number, page_idx in df_gutters.groupby("page_number", sort=False).indices.items():
        ...
        # ---- flank contents (n x W broadcasts) ----
        if widx is not None:
            fy0, fy1 = gy0 + _FLANK_Y_PAD, gy1 - _FLANK_Y_PAD
            w_yov = (wy0[None, :] < fy1[:, None] - _EPS) & (wy1[None, :] > fy0[:, None] + _EPS)

            # Which side of each gutter each word is on, gated by y-overlap
            # with that gutter (unpadded here; padding is applied via w_yov
            # when selecting, and blocking should err towards screening).
            ...
            # A word flanks gutter i only if no other gutter k sits between
            # them at the word's own y: word left of k and k left of i (k is
            # then fully between, since gutters hold no words).  Screening is
            # per word — a short fragment beside a tall gutter must not blank
            # out the tall gutter's whole flank, only the heights it covers.
