...
def detect_navigation_blocks(lines_df: pd.DataFrame) -> pd.DataFrame:
    ...
    2. Filter out block_role = toc
    3. Check if there's a 'text' that repeats 3 or more times
    4. Mark all those rows as block_role = navigation
    ...
    Args:
        ...
        Optional columns:
            ...
            - block_role: current role of the block
    ...
    # Step 2: Filter out block_role = toc (if block_role column exists)
    if 'block_role' in df.columns:
        not_toc_mask = df['block_role'] != 'toc'
    else:
